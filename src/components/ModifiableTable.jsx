              ...
              <tr key={rowIndex} className="bg-[#fff2bf] border-b border-gray-200 text-gray-900">
                <td key="0" className="w-25 px-5">
                  <div className='flex gap-8 justify-center items-center'>
                    <PencilIcon className='h-5 w-5 cursor-pointer' onClick={() => onEditClick({index: rowIndex, row: row})} />
                    <TrashIcon className='h-5 w-5 cursor-pointer' onClick={() => onDeleteClick({index: rowIndex, row: row})} />
                  </div>
                </td>
